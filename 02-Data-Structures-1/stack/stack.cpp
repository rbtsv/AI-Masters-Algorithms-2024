...
template <class T> class Stack{
    ...
    public:
        ...
        ~Stack(){cout << "Hi from ~Stack()!\n" << "size: " << size << endl; delete[] stack_content;}
        ...
        Stack(const Stack & other){ // Stack b = a; 
            ...
            size = other.size; // this->size == (*this).size
            stack_content = new T[size];	
            head = other.head;
...
template <class T> void Stack<T>::push(T x){ //++head;  // <=> head += 1;
    ...
    else{ throw std::length_error("Stack overflow!"); }
}
...
int main(){
	Stack<double> s1, s2(20);
	for(size_t i = 0; i < 11; ++i){
        try{s1.push(i);}
        catch(std::length_error){ cout << "Stack Overflow!!!11" << endl;}
        s2.push(i);
	}
