...
class TreapDSRMQ(TreapDS):
    def __init__(self, key=None, val=None, treap_node_class=TreapNodeRMQ):
        super().__init__(key, val, treap_node_class)
...
for i, x in enumerate(read_int_list()):
    t[i+1] = x

for _ in range(read_int()):
    print("max:", max(t, key=lambda node: node.val), "min:", min(t, key=lambda node: node.val),
          "sum:", sum(node.val for node in t))         
    print(t.RMQ(*read_int_list()))
